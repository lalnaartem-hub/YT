...
### Почему после запуска видна консоль и много `info`

Это нормально для backend-сервера ASP.NET Core. Чтобы запуск был “как приложение”, сделано:
- корневой URL `/` теперь отдаёт HTML-страницу (не сырой JSON),
- добавлен обработчик `/favicon.ico` (без 404 в логах),
- уровень логов по умолчанию понижен до `Warning`.

Итого после запуска вы открываете `http://localhost:5000/` и видите UI-страницу сервера.
